...
def index():
    ...
    # print(resp)
    print('index')
#     req = 'NE1GAM4812612355'
#     resp = helper_api.device_info(req)
#     resp = helper_api.host_info()
#     print('resp: ' + str(resp))
#     if resp is None:
#             resp = 'api_error'
...
def device_log(serianlNo):
    device_log = SqlHelp().query_device_log(serianlNo)
    device_logs = {'device_logs': device_log}
    print('sql device log')
    return render_template(
        'device_log.html',
        serianlNo=serianlNo,
...
def device_config_update():
    ...
    if request.method == 'POST':
        ...
        update_resp = helper_api.device_config_update(serianl_no, phone)
        print(update_resp)
        return render_template(
            'device_api.html',
            form_device_config=form_device_config,
...
def device_restart():
    ...
    if request.method == 'POST':
        ...
        restart_resp = helper_api.device_restart(serianl_no, redo_job)
        print(restart_resp)
        return render_template(
            'device_api.html',
            form_device_config=form_device_config,
